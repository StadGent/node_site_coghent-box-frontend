...
<script lang="ts">
import { defineComponent, onMounted, computed, PropType, watch, ref } from 'vue';
...
export default defineComponent({
  ...
  setup: (props, { emit }) => {
    ...
    const curLegend = ref<any[]>(props.legend);
    ...
    const setRect = (rect: Square, entity: Result) => {
      ...
      (rect.metadata = entity.metadata), (rect.lines = { going: [], coming: [] });
    };
          ...
          entity.metadata.map((meta) => {
            if (meta.key === 'type') {
              ...
              if (type) {
                ...
                curLegend.value.push({ type: meta.value, color: rect.stroke });
    ...
    const dropSquare = (
      ...
      if (
        square.top < height &&
        square.left > left &&
        square.left < bodyWidth.value - left
      ) {
        basket.push(square);
        square.set({
        ...
        for (let j = 0; j < basket.length; j++) {
          basket[j].set({
            left: left + 20 + j * 150 + j * 20,
            top: 18,
          });
          basket[j].title?.set({ top: basket[j].top, left: basket[j].left });
        }
    ...
    onMounted(() => {
      ...
      canvas.on('object:moving', function (e: any) {
        ...
        entity.lines.going.map((line: any) => {
          line.set({
            x1: entity.left + (entity.width * entity.scaleX) / 2,
            y1: entity.top + (entity.height * entity.scaleY) / 2,
          });
        });
        entity.lines.coming.map((line: any) => {
          line.set({
            x2: entity.left + (entity.width * entity.scaleX) / 2,
            y2: entity.top + (entity.height * entity.scaleY) / 2,
          });
        });

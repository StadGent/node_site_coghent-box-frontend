...
const SchemaText = (): {
  ...
  const LoadText = (schema: TextSchema) => {
    const txtBox = CreateTextBox({ height: schema.textBoxParams.height, width: schema.textBoxParams.width, color: schema.textBoxParams.color });
    ...
    loader.load(schema.fontParams.path, function (font: any) {
      ...
      const txt_mat = new MeshBasicMaterial({ color: schema.fontParams.color || 0x02a77f });
      const txt_mesh = new Mesh(txtGeometry, txt_mat);
      txt_mesh.position.y = -schema.fontParams.size/2;
      txt_mesh.position.x = - schema.textBoxParams.width/2 + 0.5;

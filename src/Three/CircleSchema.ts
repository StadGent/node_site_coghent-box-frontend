import { Vector2, Vector3, Mesh, CircleGeometry, MeshBasicMaterial, Line, BufferGeometry, LineBasicMaterial } from 'three';
...
const SchemaCircle = (): {
  ...
  CreateCircles: (schemas: Array<CircleSchema>) => Array<Mesh<CircleGeometry, MeshBasicMaterial>>;
  ...
  const CreateCircle = (schema: CircleSchema)=> {
    const circle = baseChapes.DrawCircle(schema.params.radius, schema.params.color || 0x02a77f, schema.params.segments || 50);
  ...
  const CreateOuterCircle = (radius: number) => {
    ...
    for (let i = 0; i <= 360; i++) {
      const pos = circleHelper.CalculatePointOfCircle({ angle: i, radius: radius } as CirclePoint);
      points.push({
        x: pos.x,
        y: pos.y,
      } as Vector2);
